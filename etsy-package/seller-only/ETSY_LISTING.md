# Etsy listing copy — Micro:bit Remote Builder
...
Bluetooth Web App for BBC micro:bit V1 or V2 + Illustrated Guide | 8 Tabs, 5 Robots, Live Graphs | STEM Classroom | Digital Download

> Product name in your Etsy shop is **"Micro:bit Remote Builder"** — use the phrase **"for the BBC micro:bit V1 or V2"** everywhere else to stay within the Micro:bit Educational Foundation's trademark-use guidelines.
...
> **Browser-based Bluetooth control panel for the BBC micro:bit V1 or V2. No install. Works on Chromebook. Chrome or Edge required — not iPhone / iPad / Safari.**
>
> ✅ **Tested on Chromebook** — primary target device for schools. Installs as a PWA, works offline after the first load.
>
> 🎮 **Micro:bit Remote Builder** turns your micro:bit into an interactive STEM lab — no installs, no accounts, no cloud. Just open it in your browser and connect.
>
...
>
> ⚙️ **Requires**:
> • BBC micro:bit V1 or V2 (sold separately)
...
**What makes Micro:bit Remote Builder different?** Side-by-side with the two free alternatives most teachers try first:

| Feature | Micro:bit Remote Builder | MakeCode Editor | Raw Web Bluetooth demos |
|---|:---:|:---:|:---:|
...
Because this is an instant download, I can't offer refunds once the ZIP has been delivered — please double-check compatibility (Chrome or Edge, desktop, BBC micro:bit V1 or V2) before buying. If the product doesn't work as described on a supported browser, message me within 14 days and I'll make it right.
...
>
> A printable A3 **BBC micro:bit V1 or V2 pin-out poster** — the exact diagram used in the Micro:bit Remote Builder illustrated guide. Labeled pins, accelerometer axes, sound sensor callout, touch-logo arrow, radio + BLE note.
>
...
>
> 💬 Loved it? Check out the full **Micro:bit Remote Builder** app in my shop — the browser Bluetooth control panel this poster was designed for.
...
> Hi [First Name] — thank you for grabbing Micro:bit Remote Builder with the launch code! 🎉
>
...
| #2 | Français | Translate EN title, keep "for the BBC micro:bit V1 or V2" English | From `etsy-playbook-fr.html` | Same footage, FR end-card text |
...
```
Hey {{BUYER_FIRST_NAME}} — thanks so much for grabbing Micro:bit Remote Builder! 🎮
...
>
> 📘 **Start here:** unzip the file and open **SETUP.md** (5-minute guide). You'll need a BBC micro:bit V1 or V2 and Chrome or Edge — Safari and iPhones won't work (that's a Web Bluetooth limitation, not the app).
>
...
>
> 1. Do you have a **BBC micro:bit V1 or V2** (the one with the gold microphone)? V1 is missing the sound sensor and a couple of features.
...
> Hi! Just checking in — were you able to get the Micro:bit Remote Builder running this week? If you ran into anything, reply here and I'll sort it out. If it worked smoothly, a star rating on the listing helps me keep building tools like this. Either way, thanks for the support!
...
| 0:12 – 0:15 | End card: logo + text overlay "Micro:bit Remote Builder — works in Chrome / Edge" | Hold for 3 seconds, this is the loop seam |
...
> *(0:00)* "This is the Micro:bit Remote Builder. It runs in your browser — no installs."
>
...
A weekly 5-minute "what you can build with Micro:bit Remote Builder" video, ending with a soft *"if you want this app, link's in the description"*. Channel takes 6 months to compound but pays forever. Skip if you don't enjoy being on camera.
...
**Why:** One site-license sale = 15 individual sales. Same files, different LICENSE. Zero extra work once the ZIP is built. Buyer file: `~/Desktop/microrxy-v1.0-site-license.zip` (already built).

### Title

```
School Site License for the BBC micro:bit V1 or V2 Bluetooth Web App | Up to 30 Teachers, Unlimited Students | STEM Classroom | Instant Digital Download
```
...
> • The full Micro:bit Remote Builder web app (runs locally in any Chromebook / laptop)
> • Ready-to-flash firmware (makecode.ts) for BBC micro:bit V1 or V2
...
>
> ⚙️ **Requires:** BBC micro:bit V1 or V2 (sold separately) + Chrome or Edge. Not supported: Safari, Firefox, iPhone, iPad (Web Bluetooth limitation).
>
...
> Thank you for licensing Micro:bit Remote Builder for your school! 🏫
>
...
**Files:** same ZIP as the single-user listing (`microrxy-v1.0.zip`). The UI auto-switches to Arabic on first launch if the browser locale is `ar-*`, or the user picks it from the language dropdown.
...
```
🎮 لوحة تحكم مايكروبت — تطبيق متكامل عبر البلوتوث للوحة BBC micro:bit V1 or V2
...
⚙️ يتطلب:
• لوحة BBC micro:bit V1 or V2 (تُشترى بشكل منفصل)
...
📘 ابدأ من هنا: فك ضغط الملف وافتح SETUP.md (دليل 5 دقائق). تحتاج لوحة BBC micro:bit V1 or V2 ومتصفح Chrome أو Edge — لا يعمل على Safari أو iPhone (قيود Web Bluetooth).
...
• 1× 4 GB USB stick containing the complete Micro:bit Remote Builder app
• 1× Pre-flashed BBC micro:bit V1 or V2 (firmware already loaded, shows the X icon on first power-on)
...
| BBC micro:bit V1 or V2 (wholesale from PiHut/ElecFreaks) | $12.00 |
...
#### 3a. Listing title
```
Micro:bit Remote Builder — District-Wide STEM Licensing | Quote for 3+ Schools | Includes Teacher Training Materials
```
...
|-------|-------|-------------|
| 1 | **Cover** | Logo + "Micro:bit Remote Builder — District-Wide Licensing Proposal for [District Name]" + your name + date |
...
| 3 | **What it is** | Screenshot of the app + tab map. One sentence: "A browser-based Bluetooth control panel for BBC micro:bit V1 or V2 — no install, no account, no cloud." |
...
My name is [Your Name], and I am the developer of Micro:bit Remote Builder — a browser-based STEM tool already in use by educators in [countries]. I am writing to propose a District-Wide License for [District Name].
...
2. Stripe Dashboard → **Products** → add `Micro:bit Remote Builder v9` (placeholder for next release).
...
Last year you bought Micro:bit Remote Builder on Etsy (order #[X]). Thank you! I just shipped v9 — it has [new feature 1], [new feature 2], and [new feature 3].
...
- 4 certificate styles: beginner ("I made my first LED"), intermediate ("I built a sensor dashboard"), advanced ("I built a robot"), mastery ("Micro:bit Remote Builder Certified")
...
Description:
🏆 Reward your students — 4 printable certificates for Micro:bit Remote Builder milestones.
• I Made My First LED
• Sensor Scientist
• Robot Builder
• Playground Master
...
Perfect as end-of-workshop rewards, homeschool milestones, or parent-facing proof of progress. Works standalone — no app purchase required, but designed to pair with our Micro:bit Remote Builder.
...
Want to plot the temperature, record sessions, and export CSV data without writing any extra code? My browser-based **Micro:bit Remote Builder** does all of that out of the box (it's what I used to take the screenshots in this tutorial). You can find it here: [Etsy URL].
...
I'm the developer of Micro:bit Remote Builder — a browser-based app that turns a bare micro:bit V2 into an interactive STEM lab (LEDs, sensors, servos, graphs, 3D). You can see it at [Etsy URL].
