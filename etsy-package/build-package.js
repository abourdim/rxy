/**
 * Micro:bit Remote Builder — Etsy Package Builder
 ...
 *
 * Produces:
 *   etsy-package/MicrobitRemote-v1.0.0/          (folder)
 *   etsy-package/MicrobitRemote-v1.0.0.zip       (final ZIP)
...
const ZIP_DIR = resolve(PKG, `MicrobitRemote-${VERSION}`);
...
async function main() {
  console.log(`\n🎮 Micro:bit Remote Builder Etsy Package Builder — ${VERSION}\n`);
  ...
  const zipPath = join(PKG, `MicrobitRemote-${VERSION}.zip`);
  if (existsSync(zipPath)) rmSync(zipPath);
  let zipped = false;
  try { execSync(`cd "${PKG}" && zip -r "${zipPath}" "MicrobitRemote-${VERSION}/"`, { stdio: 'pipe' }); zipped = true; } catch {}
  if (!zipped) try { execSync(`cd "${PKG}" && ditto -c -k --keepParent "MicrobitRemote-${VERSION}" "${zipPath}"`, { stdio: 'pipe' }); zipped = true; } catch {}
